...
description: RFIC/ISSCC technical-presentation style guidance plus a safe PPTX editing pipeline. Load before creating or modifying a technical presentation.
---

# 〇、样本与置信度

- The public repository distributes reusable layout rules and style profiles, not source decks, extracted conference corpus files, or rendered samples.
- The included defaults are a practical RFIC-style baseline. Treat them as editable guidance, select one profile per project, and validate the result against the project's own source material.
- Any conference branding, logos, or proprietary visual assets must be supplied separately by the user with appropriate rights.
...
- 页脚色带: y≈502–540pt 全宽青绿渐变（顶部黄绿细条）；使用纯渐变色带，并仅添加具有合法授权的品牌素材。
- 页码和会话号是可选项目内容，但必须在整套演示文稿中保持一致。
- 会议 logo 有意不包含在公开包中。
...
- `scripts/make_footer_band.py`：在本地生成 `assets/footer_band.png` 纯渐变页脚色带；生成文件不入库。
...
- 页脚 logo（IEEE/MTT-S/RFIC）为第三方品牌素材，不入库；确认授权后才可由用户放入本地 `assets/` 并使用。
